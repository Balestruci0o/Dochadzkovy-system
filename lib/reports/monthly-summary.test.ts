 ...
 * na reálnej dev DB (OTAZKY.md #81) — tento súbor to zamyká ako automatický
...
let empInactiveWorkedId: string;
let empInactiveNoDataId: string;
...
beforeAll(async () => {
  ...
  // Bug fix — NEAKTÍVNY (dnes ukončený) zamestnanec, ktorý v marci 2027 REÁLNE
  // pracoval (napr. import historickej dochádzky, medzičasom ukončený). MUSÍ
  // sa vo výkaze zjaviť napriek is_active=false — kritérium je "mal dáta v
  // tomto mesiaci", nie "je dnes aktívny".
  const [empInactiveWorked] = await adminDb
    .insert(employees)
    .values({ orgId, firstName: "Neaktívny", lastName: "AlePracoval", hiredOn: "2024-01-01", terminatedOn: "2027-03-31", isActive: false })
    .returning();
  empInactiveWorkedId = empInactiveWorked.id;
  await adminDb.insert(employeeWorkplaces).values({ employeeId: empInactiveWorkedId, workplaceId });
  await adminDb.insert(attendanceDays).values([
    { employeeId: empInactiveWorkedId, workplaceId, date: "2027-03-12", workedHours: "8.000", overtimeHours: "0", weekendHours: "0", holidayHours: "0", nightHours: "0", status: "done" },
  ]);

  // Bug fix — NEAKTÍVNY zamestnanec BEZ akýchkoľvek dát v marci 2027 (aj keby
  // formálne "prešiel" hiredOn/terminatedOn rozsahom) — NESMIE sa vo výkaze
  // zjaviť, žiadny prázdny riadok s nulami.
  const [empInactiveNoData] = await adminDb
    .insert(employees)
    .values({ orgId, firstName: "Neaktívny", lastName: "BezDát", hiredOn: "2024-01-01", terminatedOn: "2027-03-31", isActive: false })
    .returning();
  empInactiveNoDataId = empInactiveNoData.id;
  await adminDb.insert(employeeWorkplaces).values({ employeeId: empInactiveNoDataId, workplaceId });
});
...
describe("buildMonthlySummary — marec 2027, Hotel", () => {
  ...
  it("totals agreguje naprieč zamestnancami", async () => {
    const summary = await buildMonthlySummary(adminDb, workplaceId, 2027, 3);
    // 32 (Plná Mzda) + 24 (Fixný Plat, evidované aj keď mzdu neurčujú) + 8 (Neaktívny AlePracoval) = 64.
    expect(summary.totals.workedHours).toBeCloseTo(64, 6);
    // 362 (hodinový) + 950 (fixný) + 0 (fixný bez histórie) + 0 (neaktívny, bez sadzby) = 1312.
  ...
  it("BUG FIX — neaktívny (is_active=false) zamestnanec, čo v tomto mesiaci REÁLNE pracoval, SA ZJAVÍ vo výkaze", async () => {
    const summary = await buildMonthlySummary(adminDb, workplaceId, 2027, 3);
    const e = summary.employees.find((x) => x.employeeId === empInactiveWorkedId);
    expect(e).toBeDefined();
    expect(e!.workedHours).toBeCloseTo(8, 6);
  });

  it("BUG FIX — neaktívny zamestnanec BEZ dát v tomto mesiaci sa NEZJAVÍ (žiadny prázdny riadok s nulami)", async () => {
    const summary = await buildMonthlySummary(adminDb, workplaceId, 2027, 3);
    expect(summary.employees.some((e) => e.employeeId === empInactiveNoDataId)).toBe(false);
  });

  it("BUG FIX — ten istý neaktívny zamestnanec v mesiaci BEZ dát (apríl) sa nezjaví, aj keď v marci pracoval", async () => {
    const summary = await buildMonthlySummary(adminDb, workplaceId, 2027, 4);
    expect(summary.employees.some((e) => e.employeeId === empInactiveWorkedId)).toBe(false);
  });
});
