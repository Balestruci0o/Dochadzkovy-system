...
import { changeRateAction, type ActionState } from "@/app/(app)/zamestnanci/[id]/actions";
...
export function RateHistorySection({
  ...
  const [state, formAction, pending] = useActionState(changeRateAction, initialState);
  const today = new Date().toISOString().slice(0, 10);

  return (
    <div className="rounded-[14px] border border-line bg-paper p-5 shadow-sm">
      <div className="mb-3 flex items-center justify-between">
        <h3 className="font-serif text-base font-bold text-ink">História sadzieb</h3>
        {canEdit && (
          <button
            type="button"
          ...
          >
            {showForm ? "Zrušiť" : "Zmeniť sadzbu"}
          </button>
        )}
      </div>

      {!canEdit && (
        <p className="mb-2 text-xs text-ink-faint">Sadzbu smie meniť len majiteľ alebo manažér s pravomocou na mzdy.</p>
      )}
      ...
      <p className="mb-2 text-xs text-ink-faint">
        Výkaz z minulého mesiaca vždy použije sadzbu platnú v ten deň — staré záznamy sa nikdy neprepisujú.
      </p>
        ...
        <tbody>
          {history.map((h) => (
            <tr key={h.id} className="border-b border-line-soft last:border-none">
              <td className="py-2 text-ink-soft">{h.workplaceName ?? "všetky prevádzky"}</td>
              <td className="py-2 text-right font-semibold tabular-nums text-ink">{fmtEur(h.hourlyRate)}/h</td>
          ...
          {history.length === 0 && (
            <tr>
              <td className="py-3 text-ink-faint">Zatiaľ žiadna sadzba.</td>
            </tr>
          )}
        </tbody>
      </table>
